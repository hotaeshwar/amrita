...
const BusinessCard = () => {
  ...
  const shareHTMLCard = async () => {
    if (loading) return;
    
    setLoading(true);
    try {
      showNotification('Generating HTML Business Card...', 'success');
      ...
      // Generate vCard data for QR code (no photo, to keep QR code size small and scan-able)
      const vCardForQR = generateVCardData();
      const encodedVCardForQR = encodeURIComponent(vCardForQR);

      // Generate vCard data for downloading contact (with photo)
      const vCardForSave = generateVCardData(photoBase64);
      const encodedVCardForSave = encodeURIComponent(vCardForSave);
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body {
      ...
      background: linear-gradient(135deg, #0A1116 0%, #111C24 100%);
      min-height: 100vh;
      display: flex;
      align-items: center;
    ...
    .photo {
      ...
      object-fit: cover;
      display: block;
      box-shadow: 0 8px 20px rgba(255, 211, 0, 0.2);
    }
    .name {
      font-size: 26px;
      font-weight: 800;
    ...
    .contact-item {
      ...
      padding: 14px 18px;
      background: #1A2A36;
      border: 1px solid rgba(255, 211, 0, 0.05);
      border-radius: 16px;
      margin-bottom: 12px;
    ...
    .btn-primary:hover {
      transform: translateY(-2px);
      box-shadow: 0 8px 24px rgba(255, 211, 0, 0.4);
      background: #FFE033;
    }
    .btn-secondary {
      background: #1A2A36;
    ...
    .btn-secondary:hover {
      ...
      box-shadow: 0 8px 24px rgba(255, 211, 0, 0.15);
    }
    .footer {
      text-align: center;
      padding: 18px;
...
<body>
  <div class="business-card">
    ...
    <div class="qr-section">
      ...
      <div class="action-buttons">
        ...
        ${formData.phone ? `
        <a href="tel:${formData.phone}" class="btn-secondary">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/>
          </svg>
          Call Contact
        ...
        ` : ''}
        ${formData.website ? `
        <a href="https://${formData.website.replace(/https?:\/\//i, '')}" target="_blank" class="btn-secondary">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <circle cx="12" cy="12" r="10"/>
            <line x1="2" y1="12" x2="22" y2="12"/>
            <path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/>
  ...
  <script>
    window.addEventListener('DOMContentLoaded', function() {
      if (window.QRious) {
        const vCardData = decodeURIComponent('${encodedVCardForQR}');
        new QRious({
    ...
    function saveContact() {
      const vCardData = decodeURIComponent('${encodedVCardForSave}');
      const blob = new Blob([vCardData], { type: 'text/vcard;charset=utf-8' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      ...
      if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: `${formData.name} - Interactive Business Card`,
          text: `Here is the interactive business card of ${formData.name}. Open the attached HTML card to save contact, call contact, or visit the website.`
        });
        showNotification('HTML card shared successfully!', 'success');
      } else {
        // Fallback: Download
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        ...
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
        showNotification('HTML Card downloaded! Share it manually on WhatsApp.', 'success');
      }
    } catch (err) {
